/* $Id: carrier_tews_pci.c 110 2007-09-18 13:52:57Z Hesse $" */
/******************************************************************************
*******************************************************************************
**                                                                           **
**                                                                           **
...
**                                                                           **
**    File             carrier_tews_pci.c                                    **
**                                                                           **
**    Function         Driver source for TEWS PCI carrier boards             **
**                                                                           **
**                                                                           **
...
#undef DEBUG_CARRIER
#define DEBUG_NAME          "TEWS_PCI : "

#define DRIVER_NAME         "TEWS TECHNOLOGIES - (Compact)PCI IPAC Carrier"
...
MODULE_DESCRIPTION("TEWS (Compact)PCI IPAC Carrier Driver");
#endif
...
struct carrier_board_info {
    ...
    char  *iface_reg;
        /* pointer to the carrier board ip interface register (BAR2) */

    char  *ip_space;
        /* pointer to the carrier board IO, ID and INT spaces        */

    struct resource *bar[TxPCI_NUM_BAR];
        /* allocated resources by this driver                        */

    struct carrier_slot  slot[TxPCI_MAX_SLOT];
};
...
static unsigned short status_timeout[] = {
    TxPCI_A_TIMEOUT, 
    TxPCI_B_TIMEOUT, 
    TxPCI_C_TIMEOUT, 
    TxPCI_D_TIMEOUT
...
static unsigned short status_error[] = {
    TxPCI_A_ERROR, 
    TxPCI_B_ERROR, 
    TxPCI_C_ERROR, 
    TxPCI_D_ERROR
...
static unsigned short status_int0[] = {
    TxPCI_A_INT0,
    TxPCI_B_INT0,
    TxPCI_C_INT0,
    TxPCI_D_INT0
...
static unsigned short status_int1[] = {
    TxPCI_A_INT1,
    TxPCI_B_INT1,
    TxPCI_C_INT1,
    TxPCI_D_INT1
...
static unsigned short ip_reset[] = {
    TxPCI_A_RESET, 
    TxPCI_B_RESET, 
    TxPCI_C_RESET, 
    TxPCI_D_RESET
 ...
 */
#define MAX_REJECT_PCI_DEVICES     4
...
static PciDeviceStruct rejectedPciDevices[MAX_REJECT_PCI_DEVICES] = {  {0x00, 0x0f, 0x00}, 
...
static int enable_module_intr(struct carrier_slot *slot, int intr_line)
{
    ...
    value = readw((unsigned short*)(info->iface_reg + ctrl_reg[slot->slot_index]));
    ...
    case 1:
        ...
        break;
    default:
        return -1;      /* unsupported interrupt line */
    }

    writew(value, (unsigned short*)(info->iface_reg + ctrl_reg[slot->slot_index]));
...
static int setup_intr_detection (struct carrier_slot *slot, int intr_line, int sensitive)
{
    ...
    switch (intr_line) {
    case 0:
        if (sensitive & IPAC_EDGE_SENS) 
            value |= TxPCI_INT0_EDGE;
        else
            value &= ~TxPCI_INT0_EDGE;
        break;
    case 1:
        if (sensitive & IPAC_EDGE_SENS) 
            value |= TxPCI_INT1_EDGE;
        else
            value &= ~TxPCI_INT1_EDGE;
        break;
    default:
 ...
 *
 *  Returns 0 on success of -1 on failure
 *****************************************************************************/
 ...
 *
 *  Returns 1 if a error interrupt has occurred or 0 if not
 *****************************************************************************/
static int check_error_status (struct carrier_slot *slot)
{
    ...
    value = readw((unsigned short*)(info->iface_reg + TxPCI_STATUS_REG));

	/*  clear error interrupt status */
	writew(status_error[slot->slot_index], (unsigned short*)(info->iface_reg + TxPCI_STATUS_REG));	

	return ((value & status_error[slot->slot_index]) ? 1 : 0);
}
...
static int check_timeout_status (struct carrier_slot *slot)
{
	...
	writew(status_timeout[slot->slot_index], (unsigned short*)(info->iface_reg + TxPCI_STATUS_REG));

	return ((value & status_timeout[slot->slot_index]) ? 1 : 0);
}
...
static int interrupt_ack (struct carrier_slot *slot, int intr_line)
{
    ...
    switch (intr_line) {
    case 0:
		writew(status_int0[slot->slot_index], (unsigned short*)(info->iface_reg + TxPCI_STATUS_REG));
		return readb((unsigned char*)(info->ip_space + TxPCI_INT_SPACE_OFF + TxPCI_INT_SPACE_GAP * slot->slot_index));
		break;

	case 1:
		writew(status_int1[slot->slot_index], (unsigned short*)(info->iface_reg + TxPCI_STATUS_REG));
		return readb((unsigned char*)(info->ip_space + 2 + TxPCI_INT_SPACE_OFF + TxPCI_INT_SPACE_GAP * slot->slot_index));
		break;
 ...
 *
 *  This function returns 0 on success or -1 if arguments are bad
 *****************************************************************************/
static int reset_carrier (struct carrier_slot *slot)
{
    ...
    value = readw((unsigned short*)(info->iface_reg + TxPCI_RESET_REG));
    
	/* reset all slots */
	value |= TxPCI_A_RESET | TxPCI_B_RESET | TxPCI_C_RESET | TxPCI_D_RESET;

    writew(value, (unsigned short*)(info->iface_reg + TxPCI_RESET_REG));
...
static int reset_slot (struct carrier_slot *slot)
{
	...
	value |= ip_reset[slot->slot_index];
...
static int carrier_init_one(struct pci_dev *dev, const struct pci_device_id  *id)
{
    ...
    unsigned long  mem8_base;
    unsigned long  mem16_base;
    ...
    */
    ...
    for (i=2; i<6; i++) { 
        if ((info->bar[i] = request_mem_region(pci_resource_start(dev, i), pci_resource_len(dev, i), "TPCI200/100")) == 0) {
    ...
    ioidint_base = pci_resource_start(dev, 3);
    mem16_base = pci_resource_start(dev, 4);
    mem8_base = pci_resource_start(dev, 5);

    /* initialize carrier board info structure */
    info->dev = dev;
    info->iface_reg = (char*)ioremap(pci_resource_start(dev, 2), TxPCI_IFACE_SIZE);
    info->ip_space  = (char*)ioremap(pci_resource_start(dev, 3), TxPCI_IOIDINT_SIZE);
    ...
    for (i=0; i<id->driver_data; i++) {
        ...
        info->slot[i].IO_space.physical_address     = (void*)(ioidint_base + TxPCI_IO_SPACE_OFF + TxPCI_IO_SPACE_GAP*i);
        info->slot[i].IO_space.space_size           = TxPCI_IO_SPACE_SIZE;
        info->slot[i].ID_space.physical_address     = (void*)(ioidint_base + TxPCI_ID_SPACE_OFF + TxPCI_ID_SPACE_GAP*i);
        info->slot[i].ID_space.space_size           = TxPCI_ID_SPACE_SIZE;
        info->slot[i].MEM8_space.physical_address   = (void*)(mem8_base + TxPCI_MEM8_GAP*i);
        info->slot[i].MEM8_space.space_size         = TxPCI_MEM8_SIZE;
        info->slot[i].MEM16_space.physical_address  = (void*)(mem16_base + TxPCI_MEM16_GAP*i);
        info->slot[i].MEM16_space.space_size        = TxPCI_MEM16_SIZE;   
    }
...

{
    ...
    for (i=0; i<TxPCI_NUM_BAR; i++) {
        if (info->bar[i]) release_mem_region(pci_resource_start(info->dev, i), pci_resource_len(info->dev, i));
    }

    if (info->iface_reg) iounmap(info->iface_reg);
    if (info->ip_space) iounmap(info->ip_space);
...
**  This driver supports a wide range o TEWS PCI and Compact PCI carrier boards. These boards are
**  quite identical the only difference is the number of IP slots. The number of
**  IP slots will be passed by the last parameter "driver_data" to the probe function.
*/
...
#define TPCI100_VENDOR_ID       0x1498
#define TPCI100_DEVICE_ID       0x3064
#define TPCI100_SUBVENDOR_ID    0x1498
#define TPCI100_SUBDEVICE_ID    0x300A

#define TCP201_VENDOR_ID        0x1498
#define TCP201_DEVICE_ID        0x20C9
#define TCP201_SUBVENDOR_ID     0x1498
#define TCP201_SUBDEVICE_ID     0x200A

#define TCP211_VENDOR_ID        0x1498
#define TCP211_DEVICE_ID        0x20D3
#define TCP211_SUBVENDOR_ID     0x1498
#define TCP211_SUBDEVICE_ID     0x200A

#define TCP212_VENDOR_ID        0x1498
#define TCP212_DEVICE_ID        0x20D4
#define TCP212_SUBVENDOR_ID     0x1498
#define TCP212_SUBDEVICE_ID     0x200A

#define TCP213_VENDOR_ID        0x1498
#define TCP213_DEVICE_ID        0x20D5
#define TCP213_SUBVENDOR_ID     0x1498
#define TCP213_SUBDEVICE_ID     0x200A

#define TCP220_VENDOR_ID        0x1498
#define TCP220_DEVICE_ID        0x20DC
#define TCP220_SUBVENDOR_ID     0x1498
#define TCP220_SUBDEVICE_ID     0x200A


static struct pci_device_id  carrier_pci_table[] = {
    { TPCI200_VENDOR_ID, TPCI200_DEVICE_ID, TPCI200_SUBVENDOR_ID, TPCI200_SUBDEVICE_ID, 0, 0, 4 },
    { TPCI100_VENDOR_ID, TPCI100_DEVICE_ID, TPCI100_SUBVENDOR_ID, TPCI100_SUBDEVICE_ID, 0, 0, 2 },

    { TCP201_VENDOR_ID, TCP201_DEVICE_ID, TCP201_SUBVENDOR_ID, TCP201_SUBDEVICE_ID, 0, 0, 4 },
    { TCP211_VENDOR_ID, TCP211_DEVICE_ID, TCP211_SUBVENDOR_ID, TCP211_SUBDEVICE_ID, 0, 0, 2 },
    { TCP212_VENDOR_ID, TCP212_DEVICE_ID, TCP212_SUBVENDOR_ID, TCP212_SUBDEVICE_ID, 0, 0, 2 },
    { TCP213_VENDOR_ID, TCP213_DEVICE_ID, TCP213_SUBVENDOR_ID, TCP213_SUBDEVICE_ID, 0, 0, 2 },
    { TCP220_VENDOR_ID, TCP220_DEVICE_ID, TCP220_SUBVENDOR_ID, TCP220_SUBDEVICE_ID, 0, 0, 4 },
    {0, }
};
