...
static int icpmulti_init(void)
{
  ...
  /* init mutexes */
  //rt_typed_sem_init(&private.ai_sem,1,RES_SEM);
  //rt_typed_sem_init(&private.ao_sem,1,RES_SEM);
  //rt_typed_sem_init(&private.di_sem,1,RES_SEM);
  //rt_typed_sem_init(&private.do_sem,1,RES_SEM);
  ...
  //icpmulti_write_ao(0,2048);
  //icpmulti_write_ao(1,2048);
...
#ifdef ICP_MULTI_EXTDEBUG
    		printk("icp multi B n=%d ST=%4x\n",n,readw(private.iobase+ICP_MULTI_ADC_CSR));
#endif
...
#ifdef ICP_MULTI_EXTDEBUG
	printk("icpmulti read AI chan=%d value=%d\n",channel,*value);
	printk("icp multi EDBG: END: icp_multi_insn_read_ai(...) n=%d\n",n);
#endif
...
int icpmulti_write_ao(int channel, short value)
{
	...
	for (n=0; n<1; n++) {
		// Wait for analogue output data register to be ready for new data, or get fed up waiting
			...
			//#ifdef ICP_MULTI_EXTDEBUG
			if (!(timeout%10))
			  printk("icpmulti ERROR :AO channel%d n=%d tm=%d ST=%4x\n",channel,
				 n,timeout,readw(private.iobase+ICP_MULTI_DAC_CSR));
			//#endif
			rt_busy_sleep(1000);
    		}

		// If we reach here, a timeout has occurred
    		printk("D/A insn timeout");
...
#ifdef ICP_MULTI_EXTDEBUG
  printk("icp multi EDBG:  setup_channel_list()\n");
#endif
...
#ifdef ICP_MULTI_EXTDEBUG
    printk("GS: %2d. [%4x]=%4x %4x\n", i, chanprog, range, private.act_chanlist[i]);
#endif
  }
...
#ifdef ICP_MULTI_EXTDEBUG
  printk("icp_multi EDBG: BGN: init_ao_channels(...)\n");
#endif
...
#ifdef ICP_MULTI_EXTDEBUG
  printk("icp multi EDBG: END: init_ao_channels(...)\n");
#endif
  return 0;
